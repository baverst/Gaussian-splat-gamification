...
{
    [BurstCompile]
    public class GaussianSplatHierarchicalCreate : EditorWindow
    {
        const string kProgressTitle = "Creating Hierachical Gaussian Splat Asset";
        ...
        [MenuItem("Tools/Gaussian Splats/Create Hierachical GaussianSplatAsset")]
        public static void Init()
        {
            var window = GetWindowWithRect<GaussianSplatHierarchicalCreate>(new Rect(50, 50, 360, 340), false, "Hierarchical Gaussian Splat Creator", true);
        ...
        unsafe void CreateAsset(GameObject parent, string folderPath)
        {
            ...
            string baseName = Path.GetFileNameWithoutExtension(FilePickerControl.PathToDisplayString(m_InputFile)) + m_Assets;
            m_Assets++;
            ...
            GaussianSplatRenderer renderer = newSplat.AddComponent<GaussianSplatRenderer>();
            renderer.m_Asset = savedAsset;
            renderer.OnEnable();
            newSplat.transform.rotation = Quaternion.Euler(177.237f, newSplat.transform.rotation.eulerAngles.y, newSplat.transform.rotation.eulerAngles.z);
            if (parent != null)
            {
                newSplat.transform.SetParent(parent.transform);
            }
            
            foreach (string subfolder in Directory.GetDirectories(folderPath))
            {
                CreateAsset(newSplat, subfolder);
            }
